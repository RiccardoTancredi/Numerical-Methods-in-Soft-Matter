...
void MD_step()
{
	...
	*/
	...
	for (i = i_ini; i <= i_fin; i++)
	{
		...
		{
			/* add at the beginning of the cell list */
	...
	if (eps0 != 0)
	{
		...
		for (ii = 0; ii < N0; ii++)
		{
			...
			if (tau_cell[a_p][b_p] == sim_time_T)
			{
				...
				{
					for (j = 0; j < D; j++)
						v[j] = Shortest(x[0][j] - x[i][j], half_box[j], box[j]);
					r2 = Mod2(v);
					if (r2 < cut0SQ)
					{
						e = fp0 * exp(-r2 * inv0);
	...
	{ // assuming k_pol>0 if Lp>1
		for (a = 0; a < Mp; a++)
		{
			...
			for (b = 1; b < Lp; b++)
			{
				...
				for (j = 0; j < D; j++)
					vb[i][j] = v[j] = Shortest(x[ii][j] - x[i][j], half_box[j], box[j]);
				r = Mod(v);
				...
				for (j = 0; j < D; j++)
				{
					...
					/* Active matter implementation:
					harmonic spring of rest length Λ and propulsive force \vec{f}
					oriented as the vector \vec{r}_i -> \vec{r}_{i+1} of magnitude f. */

					force = (model != 2) ? k_pol * v[j] : -k_pol*(r-Lambda)*v[j]/r;
					F[ii][j] += force;
					if (model == 2) F[ii][j] += f_active*v[j]/r;
					F[i][j] -= force;
				}
				i = ii;
				ii++;
					...
					if (tau_cell[a_p][b_p] == sim_time_T)
					{
						...
						do
						{
							...
							if (ii > i)
							{
								...
								if (ii == i + 1 && BONDED[i])
									for (j = 0; j < D; j++)
										v[j] = vb[i][j]; // computed above
								else
									for (j = 0; j < D; j++)
										v[j] = Shortest(x[ii][j] - x[i][j], half_box[j], box[j]);
								/* Gaussian repulsion */
								r2 = Mod2(v);
								if (r2 < cutSQ)
								{
	...
	for (j = 0; j < D; j++)
	{
		...
		for (i = i_ini; i <= i_fin; i++)
			x[i][j] += mu_dt * F[i][j] + dB[i][j];
	}
	/* taking care of periodic boundary conditions */
	i = 0;
	for (j = 0; j < D; j++)
	{
		/* trap moved according to the probe's position, even if the trap goes outside of the box */
		while (x[i][j] < 0)
		{
			x[i][j] += box[j];
			x_trap[j] += box[j];
...
void Init()
{
	...
	x[0][0] = box[0] / 2.;
	x[0][1] = box[1] / 2.;
	x_trap[0] = (model == 2) ? x[0][0] : x[0][0] + 1.5 * v_trap * gam0 / k_trap;
	x_trap[1] = box[1] / 2.;
	...
	for (m = 0; m < Mp; m++)
	{
		for (n = 0; n < Lp; n++)
		{
			if (n == 0)
				for (j = 0; j < D; j++)
					x[i][j] = box[j] * RngStream_RandU01(rngs);
			else
			{
				for (j = 0; j < D; j++)
					x[i][j] = x[i - 1][j];
...
void Save_traj(int nrun, double ti, double v)
{
	...
	fprintf(out1, "%d %lf 0 0 \n", nrun, ti); // first useless row
	for (i = 0; i <= i_fin; i++)
		fprintf(out1, "%.4lf %.4lf %d %d\n",
...
int main(int argc, char *argv[])
{
	...
	// Check if a parameter file is provided as a command-line argument
	if (argc != 2)
	{
		fprintf(stderr, "Usage: %s parameters_file.txt\n", argv[0]);
		exit(EXIT_FAILURE);
	}

	// Read parameters from the file
	readParameters(argv[1], &nstep_save, &N, &Lp, box, &T, &v_trap_ini, &Nv, &v_per_decade,
				   &k_trap, &k_pol, &R, &eps, &R0, &eps0, &dt, &tt, &Lambda, &f_active, &model);
	...
	if (nstep_save)
	{
		fprintf(stderr, "\nSaving configurations every %d steps\n", nstep_save);
	}
